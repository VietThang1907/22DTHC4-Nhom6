 ...
 * Get all unique movie categories
 */
router.get('/movies/categories', async (req, res) => {
  try {
    // Sử dụng aggregation để lấy tất cả các category duy nhất từ phim
    const categories = await Movie.aggregate([
      // Giải nén mảng categories thành các tài liệu riêng biệt
      { $unwind: "$category" },
      
      // Nhóm theo category.id để loại bỏ trùng lặp
      { 
        $group: { 
          _id: "$category.id",
...
router.get('/movies', async (req, res) => {
  try {
    ...
    if (req.query.category || req.query.categoryId) {
      const categoryIdToFilter = req.query.categoryId || req.query.category;
      filter['category.id'] = categoryIdToFilter;
      console.log('Filtering by category ID:', categoryIdToFilter);
    }
